...
# construct the callback to save only the test model to disk based on the validation loss
checkpoint = ModelCheckpoint(args['weights'], monitor='val_loss', mode='min', save_best_only=True, verbose=1)
callbacks = [checkpoint]

print('[INFO] training network...')
H = model.fit(trainX, trainY, validation_data=(testX, testY), batch_size=64, epochs=40, callbacks=callbacks,
              verbose=2)
