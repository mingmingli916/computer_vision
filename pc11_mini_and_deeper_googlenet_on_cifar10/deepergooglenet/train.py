...
train_gen = HDF5DatasetGenerator(config.TRAIN_HDF5, batch_size=config.BATCH_SIZE, preprocessors=[sp, mp, iap], aug=aug,
                                 classes=config.NUM_CLASSES)
val_gen = HDF5DatasetGenerator(config.VAL_HDF5, batch_size=config.BATCH_SIZE, preprocessors=[sp, mp, iap],
                               classes=config.NUM_CLASSES)
...
if args['model'] is None:
    ...
    model = DeeperGoogLeNet(height=64, width=64, depth=3, classes=config.NUM_CLASSES, reg=.0002)
    opt = Adam(lr=1e-3)
